...
- 当前研究结论：关键历史、展期、资金费账本、目标数量退出和权限资料仍不完整，暂时不能判断策略是否成立。
...
- `历史资料不足`：小时历史当前只有约 21 天，共同样本不足以证明长期协整。
- `展期语义待建模`：两腿滚动日/时间不同，可能制造结构性价差变化。
- `资金费账本资料缺失`：API 的 `value`/`rate`/`direction` 尚未与账户账本核对。
- `盘口与退出资料缺失`：尚未完成目标数量盘口、部分成交和退出滑点回放。
- `账户权限资料缺失`：账户、地区和真实交易权限未核验。
