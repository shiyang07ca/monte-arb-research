...
3. **当前资料缺口是历史深度、展期状态、funding 账本、目标数量成交与退出。** 因此暂时不能判断策略是否成立，研究代码保持只读。[44][46][68][69]
...
并核对方向、周期、结算时点、预测/已结算状态以及账户实际 ledger。当前资金费 API 字段仍未和账户现金账本核对。[46][53][54]
...
任何一个关键成本或退出字段未知，都要明确写成资料缺失，不能填零。

## 5. 当前可行性判断

| 维度 | 当前判断 | 原因 |
|---|---|---|
| 经济关系 | 可继续研究 | Brent/WTI 有经济联系，但两腿是不同 RWA 永续，且期货展期不同 |
| 历史统计 | 资料缺失 | 当前共同 1h 历史约 21 天，不能证明长期协整 |
| 定价稳定性 | 资料缺失 | oracle/EMA/index/mark 与展期窗口需建模 |
| funding 收益 | 资料缺失 | API 字段到实际账户现金流尚未核对 |
| 目标数量执行 | 资料尚不完整 | 有公开盘口和成交端点，但还没有双腿进出回放 |
| 费用 | 资料尚不完整 | Standard 账户文档为 0 maker/taker，但账户等级、RFQ 和其他延迟/费用配置需确认 |
| 保证金/清算 | 资料缺失 | 有市场快照保证金字段，但账户组合保证金、清算路径和异常状态未完成审计 |
| 交易权限 | 资料缺失 | 只读公共 API 不证明账户、地区或实时交易权限 |
| 综合结论 | 暂时不能判断 | 不能承诺收益，不进入真实交易 |
...
|---|---|---|---|---|---|---|---|
| Lighter WTI–BRENTOIL 同场所 RWA 相对价值 | 中–高（待验证） | 中：官方 candles/funding/orderbook 可读，但历史深度和语义存在资料缺口 | 很高：RWA、oracle、展期、统计、执行和风险 | 中–高 | 研究阶段 `$0`；只读公共 API；实盘需账户、权限和保证金 | 展期断点、oracle/EMA、单腿、流动性、funding、清算 | **1** |
...
- 资料足够继续纸上回放：合约语义、历史数据、资金费、目标数量入口与退出、费用、保证金和失败路径均有证据，并且样本外净现金 PnL 在压力假设下仍非负；
- 暂时不成立：确认结构关系不存在、目标数量执行成本压过价差，或压力测试净现金必然为负；
- 资料缺失：关键字段未知，尤其是结算、展期、funding 账本、权限、退出或目标数量成交。

当前 WTI–BRENTOIL 仍缺少关键研究资料，但作为学习和研究案例的优先级为 **1**；这不是对未来收益的判断。
