...
第一阶段只做只读采集、数据审计、纸上现金流和执行回放；不认证、不发单、不连接私钥。当前策略状态保持 `Blocked`，真实执行保持 `No-Go`。
...
- `roll_weight_unknown`（无法从公开接口重建具体状态时）
- `price_source_unknown`

展期时段不能事后用“异常值”删除。至少要做三组结果：包含展期、排除展期、按展期阶段分层。若样本不足以比较，结论为 `Blocked`。
...
如果 `value` 的单位、结算时点或方向不能由官方规则和受控纸上账本核对，就保留 `FUNDING_LEDGER_UNKNOWN`，不填零。
...
当前动态状态表只说明公共市场快照，不证明当前账户的权限、保证金余额、地区资格、可用订单类型或真实成交路径。以上字段未核验时，`PERMISSION_UNKNOWN` 和 `DEPTH_AND_EXIT_UNKNOWN` 继续有效。
...
任何一项未知都只能产生研究信号或 `Blocked`，不能产生真实订单。
